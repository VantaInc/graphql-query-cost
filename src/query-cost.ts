...
function getDirectiveCost(
  directiveCostConfig: Maybe<Map<string, number>>,
  validationContext: ValidationContext
): number {

  const defaultCost = 1;
  // if there's no specific costs for directives, assume a default cost.
  if (!isSome(directiveCostConfig)) {
    return defaultCost;
  }

  const presentDirectives = validationContext
    .getFieldDef()
    ?.astNode?.directives?.map(({ name }) => name.value);
  // if there's no directives at all, then assume a default cost.
  if (!isSome(presentDirectives)) {
    return defaultCost;
  }

  // if there is a config for directives but this field doesn't match up with any, assume a default.
  const matches = presentDirectives
    .map((directive) => directiveCostConfig.get(directive))
    .filter(isSome)
  if (matches.length === 0) {
    return defaultCost;
  }

  // otherwise, add up the matched directive costs.
  return matches.reduce((currentCost, cost) => currentCost + cost, 0);
}
  ...
  const visitor = {
    Field: {
      enter(
        ...
        const itemCost = getDirectiveCost(directiveCostConfig,validationContext);
        return {
          ...node,
...
export function costForTesting(
  ...
  return documentCost(schema, directiveCostConfig, query, variables);
}
